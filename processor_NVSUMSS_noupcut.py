...
outfile = 'noupcut_nvsumss_patch'+str(patchdec)
...
NVarr = NVarr.transpose()[(NVarr[2]>fluxcut)].transpose()
SUarr = SUarr.transpose()[(SUarr[2]>fluxcut)].transpose()
...
if options.SOUT:
    np.savetxt(outfile+'_galaxyselectionnew.txt',allarr, delimiter="|")
...
fout.write(str(stotno)+","+str(sgallatcutno)+","+str(sn2masscutno)+","+str(sw1mprocutno)+","+str(swmproj2masscutno)+","+str(sj2masscutno)+","+str(sgbcutno)+","+str(finalno)+"\n")
...
fout.close()
os.system('mv '+ outfile+'_resultnew.txt '+outfolder)
os.system('mv '+ outfile+'_galaxyselectionnew.txt '+outfolder)
