...
parser.add_option("-p", "--preab", action= "store", type = "int", default=0, dest = "PRE", help = "Whether to evaluate without aberrations")
...
fhead = str(serno)+'_Simcrwfrd_'+str(raab)+'_'+str(decab)+'_'+str(velocab)+'_'+str(ncat)+'_'
...
def Processor_Crawford(allarr, gcut=0., sgcut=0., fhead='wtf'):
    if gcut:
    ...
    vel = c*resdip*3./(float(len(allarr[0]))*2.)
    return ra, dec, resdip, vel
...
if pre:
    for cuts in [[0,0],[5,0],[10,0],[20,0],[10,5],[10,10], [20,5], [20,10]]:
        resra, resdec, resdip, resvel = Processor_Crawford(allarr, cuts[0], cuts[1], fhead)
