...
print np.min(NVarr[2]), np.max(NVarr[2]), np.min(SUarr[2]), np.max(SUarr[2])
...
if options.SOUT:
    np.savetxt(outfile+'_galaxyselection1.txt',allarr, delimiter="|")
...
fout = open(outfile+'_result1.txt', "w")
...
fout.close()
os.system('mv '+ outfile+'_result1.txt '+outfolder)
os.system('mv '+ outfile+'_galaxyselection1.txt '+outfolder)
